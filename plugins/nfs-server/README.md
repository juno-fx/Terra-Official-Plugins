...
|----------|-------------|
| `Deployment` | Single-replica NFS server (`itsthenetwork/nfs-server-alpine`, NFSv4) exporting `/nfsshare` on port 2049 |
...
| `PersistentVolume` | **Orphaned** PV (`<release>-pv`) — `nfs.server: localhost`, `path: /nfsshare`, with `port=<nodeport>` in `mountOptions`. No PVC claims it; the storage UI picks it up automatically |
...
The PV's `nfs.server` is **`localhost`** — not a node IP. When a pod claims the PV, the kubelet on the pod's node runs `mount.nfs localhost:/nfsshare -o port=<nodeport>`. `localhost` resolves to the node itself, the NodePort rule (bound on every node) routes it to the NFS server pod, and the mount lands on the exported share. No external IP, no ingress, no DNS.
...
- Nodes must have the `nfsd` kernel module loadable (most distros ship it) — the container modprobes it at start.

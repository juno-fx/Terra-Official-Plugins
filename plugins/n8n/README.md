...
- Webhook URLs become `https://<name>.<domain>/webhook/<id>`, which is the form every managed n8n host uses, and third-party callers reach them without any extra setup. `webhooks_public` is not used in this mode
...
Point a wildcard record for `*.<domain>` at the cluster ingress address, or set `publish_dns` and let ExternalDNS create each instance's record. The Domain Manager page shows the exact record and whether it currently resolves.
...
- With `domain` set the whole instance lives on its own hostname, so the editor, the REST API and the webhooks are all served there and n8n's own login is what protects them
